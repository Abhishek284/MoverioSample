            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked) {
                    mSensorDataListener_rv = new SensorDataListener() {
                        @Override
                        public void onSensorDataChanged(SensorData sensorEvent) {
                            if (sensorEvent.type == Sensor.TYPE_ROTATION_VECTOR) {
                                android.hardware.SensorManager.getRotationMatrixFromVector(rotations, sensorEvent.values);
                                float[] tempRotations = new float[9];
                                ...
                                float yawDegrees = (float) Math.toDegrees((double) values[0]);
                                float pitchDegrees = (float) Math.toDegrees((double) values[1]);
                                float rollDegrees = (float) Math.toDegrees((double) values[2]);
                                mTextView_rvResult.setText(String.format("%.4f", yawDegrees) + "," + String.format("%.4f", pitchDegrees) + "," + String.format("%.4f", rollDegrees) + "," );

//                                if (headTrackerCallback != null) {
//                                    headTrackerCallback.onAnglesChanged(yawDegrees - 90, pitchDegrees, rollDegrees);
//                                }
                            }
//                                mTextView_rvResult.setText(String.format("%.4f", data.values[0]) + "," + String.format("%.4f", data.values[1]) + "," + String.format("%.4f", data.values[2]) + "," + String.format("%.4f", data.values[3]));
    ...
    public float[] rotations  = new float[9];
    @Override
